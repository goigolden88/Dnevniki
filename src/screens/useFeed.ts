/**
 * Данные ленты: все хранилища разом.
 *
 * Лента только читает, поэтому общего состояния с модулями ей не нужно (Р-48):
 ...
 * вместе с надгробиями, а без них у отметки удалённой позиции не было бы
 * имени.
 */
...
export function useFeed(): Feed {
  ...
  // «Идёт N-й день» у открытого эпизода считается от сегодня, а вкладка
  // установленного приложения неделями не перезапускается.
  useEffect(() => {
    function refreshDay() {
