 ...
 * слияние, ни про базу. Всё, что выше уровнем, живёт в `sync.ts` — так замена
 * GitHub на нормальный бэкенд остаётся правкой одного файла
 * (docs/02-Архитектура.md, «Правила»).
 *
...
export class GitHubError extends Error {
  ...
  /** Токен не принят. Через год после выдачи — самая вероятная причина сбоя. */
  ...
  async function explain(response: Response): Promise<GitHubError> {
    ...
    if (status === 401) {
      return new GitHubError(
        'Токен не принят. Обычная причина — истёк срок: их выдают на год. ' +
