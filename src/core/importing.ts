...
export const IMPORT_FORMAT = 'dnevniki-import'
...
export function readImportFile(text: string): Record<string, unknown> {
  ...
  if (raw.format !== IMPORT_FORMAT) {
    throw new Error(`В файле нет строки "format": "${IMPORT_FORMAT}" — это не импорт записей Дневников`)
  }
...
export function buildPrompt(specs: readonly ImportSpec[], day: DateStr): string {
  ...
  return [
    'Помоги перенести мои записи в приложение «Дневники». Ниже — описание формата, а в конце — ' +
      'мои данные: заметки, таблицы или скриншоты из других сервисов. Собери из них один JSON ' +
    ...
    '3. Оценки — числом от 1 до 10, можно с одним знаком после точки. Пять звёзд умножай на два, ' +
      'сто баллов дели на десять.',
    '4. Статусы из других сервисов: смотрю, читаю, играю, прохожу, отложено → "active"; просмотрено, ' +
      'прочитано, пройдено → "done"; брошено → "dropped"; запланировано, хочу посмотреть → "planned".',
    '5. Скриншоты списков — Шикимори, MyAnimeList, Кинопоиск, Letterboxd, Steam и похожие: бери ' +
      'название, тип, статус, оценку и даты, если они видны.',
    '6. Разделы, для которых данных нет, не пиши.',
    '7. Ответь одним блоком JSON. Если записей очень много — раздели на несколько блоков, каждый — ' +
      'полный файл в том же формате.',
    '8. После JSON отдельным списком перечисли, что не удалось разобрать или в чём сомневаешься.',
    '',
    'Разделы:',
    '',
    sections.join('\n\n'),
