...
const REPO: RepoRef = { owner: 'goigolden88', name: 'dnevniki-data', branch: 'main' }
...
describe('parseRepo', () => {
  it('разбирает «владелец/репозиторий»', () => {
    expect(parseRepo('goigolden88/dnevniki-data')).toEqual({
      owner: 'goigolden88',
      name: 'dnevniki-data',
    })
  })
  ...
  it('принимает ссылку из адресной строки и .git на конце', () => {
    expect(parseRepo('https://github.com/goigolden88/dnevniki-data')).toEqual({
      owner: 'goigolden88',
      name: 'dnevniki-data',
    })
    expect(parseRepo('https://github.com/goigolden88/dnevniki-data.git')).toEqual({
      owner: 'goigolden88',
      name: 'dnevniki-data',
    })
    expect(parseRepo('  goigolden88/dnevniki-data/  ')).toEqual({
...
describe('info', () => {
  it('видит право на запись и приватность', async () => {
    const { api, calls } = client({
      '/repos/': {
        body: {
          full_name: 'goigolden88/dnevniki-data',
          private: true,
          default_branch: 'main',
    ...
    expect(await api.info()).toEqual({
      fullName: 'goigolden88/dnevniki-data',
      private: true,
      canWrite: true,
      defaultBranch: 'main',
    })
