...
import type { CycleEvent, CycleItem, Migration } from './model.ts'
 ...
 * Подделка подключается только здесь: остальным тестам глобальный
 * `indexedDB` не нужен, и подсовывать его им незачем.
 *
...
function item(id: string, over: Partial<CycleItem> = {}): CycleItem {
  return { id, updatedAt: T1, name: 'Стрижка', cat: 'Гигиена', intervalDays: null, ...over }
}

function mark(id: string, over: Partial<CycleEvent> = {}): CycleEvent {
  return { id, updatedAt: T1, itemId: 'i1', date: '2026-09-01', ...over }
}
...
describe('запись и происхождение', () => {
  ...
  it('пачка пишется целиком и метится вся', async () => {
    await db.putMany('items', [item('i1'), item('i2'), item('i3')])
  ...
  it('get на пустом месте отдаёт undefined, а не падает', async () => {
    expect(await db.get('items', 'нет такой')).toBeUndefined()
  })
})

describe('мягкое удаление', () => {
  it('ставит надгробие, запись остаётся в базе навсегда', async () => {
    await db.put('items', item('i1'))

    expect(await db.remove('items', 'i1')).toBe(true)
    // Вычистить запись нельзя: второе устройство при следующей
    // синхронизации воскресит её (Р-07).
    expect((await db.get('items', 'i1'))?.deleted).toBe(true)
  })

  it('удалённое не попадает в списки и счётчики, но видно с includeDeleted', async () => {
    await db.putMany('items', [item('i1'), item('i2')])
    await db.remove('items', 'i1')

    expect(await db.getAll('items')).toHaveLength(1)
    expect(await db.count('items')).toBe(1)
    expect(await db.getAll('items', { includeDeleted: true })).toHaveLength(2)
    expect(await db.count('items', { includeDeleted: true })).toBe(2)
  })

  it('надгробие на пустом месте не ставится', async () => {
    expect(await db.remove('items', 'нет такой')).toBe(false)
    expect(await db.count('items', { includeDeleted: true })).toBe(0)
  })

  it('удаление двигает updatedAt и метит грязной — оно должно уехать', async () => {
    await db.putRemote('items', [item('i1', { updatedAt: T1 })])
    await db.remove('items', 'i1')

    const tombstone = await db.get('items', 'i1')
...
describe('слияние по updatedAt', () => {
  it('входящая новее — побеждает', async () => {
    await db.putRemote('items', [item('i1', { name: 'Стрижка', updatedAt: T1 })])
    const applied = await db.merge(
      'items',
      [item('i1', { name: 'Педикюр', updatedAt: T2 })],
      'remote',
    )

    expect(applied).toBe(1)
    expect((await db.get('items', 'i1'))?.name).toBe('Педикюр')
  })

  it('входящая старее — отбрасывается', async () => {
    await db.putRemote('items', [item('i1', { name: 'Педикюр', updatedAt: T2 })])
    const applied = await db.merge(
      'items',
      [item('i1', { name: 'Стрижка', updatedAt: T1 })],
      'remote',
    )
  ...
  it('ровно то же время не применяется — своё остаётся своим', async () => {
    ...
    const applied = await db.merge(
      'items',
      [item('i1', { name: 'Стрижка', updatedAt: T2 })],
      'remote',
    )
  ...
  it('незнакомая запись добавляется', async () => {
    const applied = await db.merge('items', [item('i1'), item('i2')], 'remote')
  ...
  it('надгробие новее живой записи — удаление доезжает до второго устройства', async () => {
    await db.putRemote('items', [item('i1', { updatedAt: T1 })])
    await db.merge('items', [item('i1', { updatedAt: T2, deleted: true })], 'remote')
  ...
  it('живая запись новее надгробия — правка воскрешает удалённое', async () => {
    await db.putRemote('items', [item('i1', { updatedAt: T2, deleted: true })])
    await db.merge('items', [item('i1', { name: 'Стрижка', updatedAt: T3 })], 'remote')

    // Не ошибка: на другом устройстве запись правили позже, чем здесь
    // удаляли, и по Р-07 побеждает поздняя правка.
    expect((await db.get('items', 'i1'))?.deleted).toBeUndefined()
    expect(await db.count('items')).toBe(1)
  })

  it('надгробие участвует в сравнении, а не считается отсутствием записи', async () => {
    await db.putRemote('items', [item('i1', { updatedAt: T3, deleted: true })])
    const applied = await db.merge('items', [item('i1', { updatedAt: T2 })], 'remote')
  ...
  it('слияние с сервера грязным не метит, из файла — метит', async () => {
    await db.merge('items', [item('i1')], 'remote')
    expect(await db.listDirty()).toEqual([])

    await db.merge('items', [item('i2')], 'imported')
    expect(await db.listDirty()).toHaveLength(1)
  })

  it('пустой список не трогает базу', async () => {
    expect(await db.merge('items', [], 'remote')).toBe(0)
  })
})

describe('очередь изменений', () => {
  it('пометка несёт хранилище, запись и время правки', async () => {
    const saved = await db.put('cycleEvents', mark('e1'))

    expect(await db.listDirty()).toEqual([{ store: 'cycleEvents', id: 'e1', at: saved.updatedAt }])
  })

  it('повторная правка одной записи даёт одну пометку, а не две', async () => {
    const first = await db.put('items', item('i1'))
    await pause()
    await db.put('items', { ...first, name: 'Педикюр' })
  ...
  it('правка во время отправки пометку не теряет', async () => {
    ...
    await pause()
    await db.put('items', { ...saved, name: 'Педикюр' })
  ...
  it('различает записи с одинаковым id в разных хранилищах', async () => {
    await db.put('items', item('одинаковый'))
    await db.put('cycleEvents', mark('одинаковый'))
...
describe('оповещение об изменениях', () => {
  it('говорит, что и откуда записалось', async () => {
    ...
    await db.putMany('items', [item('i1'), item('i2')])
    await db.putRemote('cycleEvents', [mark('e1')])
    off()

    expect(seen).toEqual([
      { store: 'items', origin: 'local', count: 2 },
      { store: 'cycleEvents', origin: 'remote', count: 1 },
    ])
  })
  ...
  it('пустая запись не оповещает — ноль сюда не приходит', async () => {
    ...
    await db.putMany('items', [])
    await db.merge('items', [item('i1', { updatedAt: T1 })], 'remote')
  ...
  it('упавший слушатель не роняет запись — она уже прошла', async () => {
    ...
    await expect(db.put('items', item('i1'))).resolves.toBeDefined()
    off()
...
describe('слепок', () => {
  it('включает надгробия — без них второе устройство воскресит удалённое', async () => {
    ...
    expect(snapshot.data.items).toHaveLength(2)
    expect(snapshot.data.items.find((each) => each.id === 'i1')?.deleted).toBe(true)
  })
  ...
  it('загрузка сливается, а не затирает: файл может быть старше здешнего', async () => {
    await db.putRemote('items', [item('i1', { name: 'Педикюр', updatedAt: T3 })])
    ...
    const applied = await db.importAll({
      ...
      data: {
        ...data,
        items: [item('i1', { name: 'Стрижка', updatedAt: T1 }), item('i2', { updatedAt: T2 })],
      },
    })

    expect(applied).toBe(1)
    expect((await db.get('items', 'i1'))?.name).toBe('Педикюр')
    expect(await db.get('items', 'i2')).toBeDefined()
  })

  it('свой же слепок переживает круг через файл', async () => {
    await db.putMany('items', [item('i1'), item('i2')])
    await db.put('cycleEvents', mark('e1'))
    ...
    expect(await db.count('items')).toBe(2)
    expect(await db.count('cycleEvents')).toBe(1)
  })
})
...
describe('схема базы', () => {
  it('заводит все хранилища и индексы, на которых держится остальное', async () => {
    ...
    try {
      ...
      // На updatedAt держится слияние, на date и itemId — история позиции.
      for (const store of SYNCED_STORES) {
        expect([...tx.objectStore(store).indexNames]).toContain('updatedAt')
      }
      expect([...tx.objectStore('cycleEvents').indexNames].sort()).toEqual([
        'date',
        'itemId',
        'updatedAt',
      ])
      expect([...tx.objectStore('measures').indexNames].sort()).toEqual([
        'date',
        'metric',
        'updatedAt',
      ])
